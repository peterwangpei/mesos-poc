...
RC_NAME=${RC_NAME:-"nginx"}

POD_IMAGE=${POD_IMAGE:-"kubernetes/pause"}
POD_CPU=${POD_CPU:-50}
POD_MEMORY=${POD_MEMORY:-128}
POD_COUNT=${POD_COUNT:-8}
POD_UP_COUNT=${POD_UP_COUNT:-16}
POD_DOWN_COUNT=${POD_DOWN_COUNT:-4}

KILL_POD_COUNT=${KILL_POD_COUNT:-2}
...
do
{
    ...
    ./template.sh $CLEAR_TEMPLATE $CLEAR_DEFINITION '{CASE_NAME}'/'CLEAR' '{KUBECTL}'/$(encode $KUBECTL) '{API_SERVER}'/$API_SERVER '{NAMESPACE}'/$NAMESPACE '{RC_NAME}'/$(encode $RC_NAME) '{LOG_PATH}'/$(encode $LOG_PATH) '{LOG_NAME}'/$LOG_NAME
