...
GEN_RESETSCRIPT=${GEN_RESETSCRIPT:-true}

function encode {
    echo "$(echo $1 | sed -e 's/\//\\\//g')"
}

if [ "$GEN_RESETSCRIPT" == true ]; then
    echo "======Create reset script"
    ./template.sh $RESET_TEMPLATE "reset.sh" '{API_SERVER}'/$API_SERVER '{NAMESPACE}'/$NAMESPACE '{RC_NAME}'/$RC_NAME '{KUBECTL}'/$(encode $KUBECTL) '{CONCURRENT}'/$CONCURRENT
    chmod a+x ./reset.sh
fi
